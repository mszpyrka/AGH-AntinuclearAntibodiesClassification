...
from hep2_classification.preprocessing import preprocess, _remove_tag, _reduce_channels, _resize, _denoise, _normalize, \
    _equalize_histogram
...
class TestPreprocessing(unittest.TestCase):
    ...
    def setUp(self):
        self.img = cv.imread('test-image.tif')

    def test_remove_tag(self):
        img_removed = _remove_tag(self.img)

        self.assertEqual(
            (self.img.shape[0] - 55, self.img.shape[1], self.img.shape[2]),
            img_removed.shape
        )
